...
def get_version():
    with open(f"{os.path.dirname(sys.argv[0])}/default.nix") as f:
...
def name_to_attr_path(req: str, packages: Dict[str, Dict[str, str]]) -> Optional[str]:
    ...
    if len(attr_paths) > 1:
        for to_replace, replacement in PKG_PREFERENCES.items():
            try:
                attr_paths.remove(f"{PKG_SET}.{to_replace}")
                attr_paths.add(f"{PKG_SET}.{replacement}")
            except KeyError:
                pass
    # Let's hope there's only one derivation with a matching name
    assert (
        len(attr_paths) <= 1
    ), f"{req} matches more than one derivation: {attr_paths}"
    return attr_paths.pop() if len(attr_paths) == 1 else None


def get_pkg_version(package: str, packages: Dict[str, Dict[str, str]]) -> Optional[str]:
    pkg = packages.get(f"{PKG_SET}.{package}", None)
    return None if not pkg else pkg["version"]
    ...
    for component in sorted(components.keys()):
        ...
        for req in reqs:
            ...
            if attr_path is not None:
                ...
                attr_paths.append(attr_path[len(f"{PKG_SET}."):])
            else:
                missing_reqs.append(name)
        else:
            build_inputs[component] = (attr_paths, missing_reqs)

    with open(f"{os.path.dirname(sys.argv[0])}/component-packages.nix", "w") as f:
